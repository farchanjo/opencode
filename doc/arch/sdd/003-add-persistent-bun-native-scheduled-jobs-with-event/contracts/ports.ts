/**
 * Feature 003 — Application Ports (Persistent Bun-Native Scheduled Jobs)
 *
 * These interfaces define the inbound ports owned by Feature 003. They are
 * implemented by the domain scheduler engine (`packages/core/src/jobs/**`)
 * and application adapters (`packages/opencode/src/jobs/**`), and are
 * consumed by Feature 007 operator control-plane adapters (CLI/TUI/App) per
 * ADR-0003 and the Feature 002 observation seam per ADR-0004. Feature 003
 * never registers a parallel command registry (C12) and introduces no second
 * scheduler, executor, event bus, persistence store, or notification channel
 * (ADR-0004 Decision Outcome).
 *
 * Domain: scheduler registration/reconciliation/trigger claim, canonical
 * Feature 002 Task Process creation per occurrence, bounded async
 * notification delivery over the Feature 002 observation seam, and the
 * `jobs.*` operator command/query surface. Every mutation
 * (create/update/enable/disable/delete/reschedule/run-now) requires an
 * operator principal, explicit scope, version/CAS, idempotency, and audit;
 * notification observation is read-only and control-plane wake/queue/steer
 * uses native SessionInput/SessionExecution (FR23, FR32, C9, C12).
 *
 * Wire-shape source of truth: `doc/arch/schemas/jobs/*.cue` (ids.cue,
 * enums.cue, definition.cue, schedule.cue, occurrence.cue, events.cue,
 * notification.cue — plan.md "New module tree target"). This file is the
 * TypeScript mirror; it does not redefine event payload schemas owned by
 ...
 * transitions; never coalesced or dropped (plan.md "Durable versus live
 * event classes").
 */
...
// =============================================================================
// NotificationPort — enqueue / deliverAtBoundary / ack / expire / audit
// =============================================================================
