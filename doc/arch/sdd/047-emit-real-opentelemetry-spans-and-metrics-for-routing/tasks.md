...
- [x] T001 Expose a domain-signal `offer` on the shipped export pipeline
...
- [x] T002 Add pure, SDK-free attribute mappers
...
- [x] T003 Wire routing-decision emission at the live session seam
  (`session/routing-resolve.ts`) after a decision resolves.
- [x] T004 Wire budget-consumption and breach emission at the processor budget
  seam (`session/processor.ts` step-finish).
- [x] T005 Wire fan-out admission emission at the hierarchy resolver
  (`session/routing-hierarchy.ts`) on both route (granted) and blocked (denied).
- [x] T006 Wire orchestration-outcome emission at the worker terminal fold
  (`tool/task.ts` seam) via a pure outcome mapper.
- [x] T007 Tests (`test/telemetry/routing-emitters.test.ts`): each domain emits
...
- [x] T008 Register the Feature 047 source globs in `doc/arch/speckit.toml` and
