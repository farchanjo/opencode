...
- [x] T001 Confirm the compound root cause by reading the code: the "Routing configure" modal
...
- [x] T002 Decide the read seam (ADR-0040 OPEN decision): Option A — enrich
...
- [x] T003 Part 1 — thread the Request scope into the prefill read: pass
...
- [x] T004 Part 1 — re-read on scope switch: add a deferred `createEffect` keyed on the
...
- [x] T005 Part 2 — implement the chosen effective read (Option A): make the config-status
  ...
  accordingly. (FR-A1)
- [x] T006 Part 3 — fix the field-mapping shape: add prefill extractors that read
  ...
  (FR-C1)
- [x] T007 Part 3 — verify honest placeholder vs seeded value: an absent activation seeds
  nothing → `[ ] off` / `— select —`; a present activation seeds the real value → `[x] on` /
  the mode option. (FR-C2)
- [x] T008 Part 4 — audit parity across the config-backed Configure modals: confirm `smart`
  ...
  (FR-E1)
- [x] T009 Part 4 — realtime refresh: confirm the modal + backing operator status card reflect
  ...
  store. (FR-D1)
- [x] T010 Add the tests: a TUI prefill-parity test (seeded field set == persisted activation;
...
- [x] T011 Author/finalize the speckit corpus (`spec.md`, `plan.md`, `tasks.md`) and
