...
- [ ] T001 [P] Add the pure aggregate value objects to a new session-layer module
...
- [ ] T002 Extend `RoutingSessionState` (`session/routing-state.ts` :35-43) with a
  ...
  FR-E3).
- [ ] T003 At the spawn seam (`tool/task.ts` :262-270, beside
  ...
  (FR-A1, FR-A4).
- [ ] T004 On a child terminal result (`tool/task.ts` :346-355 background /
...
- [ ] T005 Add a pure `managerCompletionGate(aggregate)` over the roster reusing
...
- [ ] T006 Wire the gate at the Manager turn-boundary in `processor.ts`
...
- [ ] T007 [P] Add a pure ordered `workerValidationChain(result, childSnapshot)` —
  ...
  (FR-C1, FR-C2).
- [ ] T008 Hook the chain at result acceptance (`tool/task.ts` :424-435), AFTER
...
- [ ] T009 Add a coalesced, event-driven wake keyed by child session id: subscribe
...
- [ ] T010 Bound each delegated Worker with a `MAX_WAIT_MS` plan constant
...
- [ ] T011 Gate the whole contract (aggregate, gate, chain, wake) on
...
- [ ] T012 Prove composition with F043 admission: a fan-out-denied Worker
...
- [ ] T013 Wrap every leaf so any defect degrades to today's ungated behavior and
...
- [ ] T014 Regression coverage for FR-F3 (a)-(h): N-Worker aggregate; pending-gate
