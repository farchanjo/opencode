...
### P1 — Smart brain and manager mode

- As a user, I want the primary context to plan, decompose, negotiate intent and
  constraints, dispatch specialists, validate results, and report outcomes so that
  manager-only policy prevents it from directly performing project work.
...
### Smart brain, manager mode, and UI (Phase 2)

27. When brain mode and Smart Agent Routing are effectively active, the primary
    context MUST operate as a manager: it MUST plan, decompose, negotiate intent and
    constraints with the router, dispatch specialist agents, validate results, and
    report outcomes. When manager-only policy is active, it MUST NOT directly execute
    project work.
28. The router MUST remain authoritative for hard gates. The brain MAY propose a
    route or request re-evaluation, but MUST NOT elevate permissions or select a
    prohibited candidate.
29. Manager-only behavior MUST reuse the existing PermissionV2, Policy, Session,
    Task, and runner lifecycle to enforce permissions and execution boundaries. It
    MUST NOT create a parallel runtime or bypass canonical authorization.
...
33. Telemetry MUST correlate brain negotiation, route proposal, accepted or rejected
    recommendation, re-route, dispatched specialist, and validation result with the
    existing session/turn/routing/execution hierarchy.
...
### Canonical command interface (Phase 2)

48. Management MUST be exposed through the canonical native TUI command registry and
    command palette opened by Ctrl+P, native slash commands, and the yargs/effectCmd
    CLI. App/Desktop MUST use their canonical command context when in scope. No
    parallel command registry is permitted. Every Smart, routing, and telemetry
    command in this feature is a native operator-only interface, never an interface
    for an LLM, brain, specialist, or tool.
...
21. **Manager-only brain.** Given brain mode, Smart Routing, and manager-only policy
     are active, when the primary context receives project work, then it records a
     plan, negotiates intent and constraints, dispatches a specialist, validates the
     result, and reports it without directly executing the project work.
...
44. **Routing dry run.** Given a routing test request, when `routing test` executes,
     then it performs no tool or mutation, reports whether a provider/decision-model
     call and external cost may occur, and records the dry-run outcome.
...
7. **Manager authorization.** Manager-only brain behavior MUST be enforced by
   canonical PermissionV2 and Policy/Session/Task authorities. A brain proposal,
   re-evaluation request, UI state, or project override MUST NOT elevate permissions
   or bypass a hard gate.
...
correlation MUST additionally cover brain negotiation, route proposal, accepted or
rejected recommendation, re-route, specialist dispatch, and validation result.
...
recommendations, re-routes, specialist dispatches, validation outcomes, capability
mismatches, serialisation decisions, and capability-overlay confidence/expiry. Metric
...
- Phase 1 is additive: telemetry is disabled or behavior-preserving until configured,
  ...
  remains available.
- Phase 2 introduces a core routing seam without treating existing plugin hooks or
  `smart_task` as equivalent lifecycle coverage. Existing plugin paths remain
  compatible and are not silently promoted to routing authority.
...
- [ADR-0002 — Core Smart Agent Routing with telemetry dependency](../../adr/0002-core-smart-agent-routing.md) — proposed core placement and routing constraints; depends on ADR-0001.
- Evidence informing both decisions: [plugin systems research note](../../research/plugin-systems.md). The research note is not itself a decision.
...
| Manager brain and contextual Smart UI | FR27–FR33 | 21–24, 33 | 2 |
...
| Secure compatible delivery | NFRs, security, migration | 2–4, 11, 13, 20, 46–52, 60–62 | 1–2 |
