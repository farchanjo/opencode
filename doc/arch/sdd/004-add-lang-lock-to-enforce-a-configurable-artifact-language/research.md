...
## Plan-phase enforcement, detection, and catalog findings

Added during the `plan` phase (ADR-0005). These notes cite current-core evidence
for the enforcement seams the plan wires; they record findings, not new decisions.

### Immutable injection and post-transform reapplication (C4)

- The mutable `experimental.chat.system.transform` hook runs after the system
  array is built for both request preparation
  ([`request.ts:56-78`](../../../../packages/opencode/src/session/llm/request.ts#L56-L78))
  and agent generation
  ([`agent.ts:368-382`](../../../../packages/opencode/src/agent/agent.ts#L368-L382)),
  and the plugin contract permits modification of system and message arrays
  ([`plugin/src/index.ts:282-296`](../../../../packages/plugin/src/index.ts#L282-L296)).
  A system-prompt string appended before the transform is therefore strippable;
  the plan reapplies the effective-language block **after** the transform at both
  seams so a plugin cannot remove it (FR17, FR25, AC7). The block is assembled
  alongside the existing V2 prompt composition
  ([`prompt.ts:1255-1286`](../../../../packages/opencode/src/session/prompt.ts#L1255-L1286)).

### Envelope metadata, never a model-controlled argument (C4)

- TaskTool accepts model-authored description/prompt/subagent type
  ([`task.ts:44-60`](../../../../packages/opencode/src/tool/task.ts#L44-L60),
  [`task.ts:197-235`](../../../../packages/opencode/src/tool/task.ts#L197-L235)),
  and WriteTool/ApplyPatch/ShellTool carry model-controlled content/patch/command
  ([`write.ts:20-38`](../../../../packages/opencode/src/tool/write.ts#L20-L38),
  [`apply_patch.ts:22-52`](../../../../packages/opencode/src/tool/apply_patch.ts#L22-L52),
  [`shell.ts:257-263`](../../../../packages/opencode/src/tool/shell.ts#L257-L263)).
  The plan stamps tag/version/source/mode onto the execution envelope and write
  context as **native immutable metadata**, never a tool argument the model can
  set or omit (FR18, FR19). Because ShellTool owns opaque heredoc/redirect
  constructs, the plan asserts explicit capability boundaries and promises no
  enforcement before generated bytes cross an OpenCode-observable boundary
  (FR23, AC18).

### Detection approach V1 — deterministic, advisory-only (C5, C6)

- No `artifactLanguage`/`artifact_language` schema field exists in the current
  packages search; policy is new config on the Config.Service merge path
  ([`config.ts:248-270`](../../../../packages/opencode/src/config/config.ts#L248-L270),
  [`config.ts:398-429`](../../../../packages/opencode/src/config/config.ts#L398-L429),
  [`config.ts:524-563`](../../../../packages/opencode/src/config/config.ts#L524-L563)).
  V1 detection is deterministic and advisory-only: it is confined to confidently
  classified prose kinds (Markdown, docs, instruction files, generated commit
  text), records only provenance/confidence-bucket/path-kind/policy-version/
  remediation-status (content-free), never blocks generic source code, and never
  gates the write or issues a translation-model call (FR16, FR20, FR21, AC8,
  AC11). Detector unknown/failure is a recorded outcome that never blocks the
  prompt, execution, or tool hot path.

### Reserved-catalog version verification (C3)

- `packages/core/src/operator/catalog.ts` is at
  `RESERVED_CATALOG_VERSION = "1.3.0"` (bumped during Feature 003) and **already**
  declares the `langlock` domain in `OPERATOR_DOMAINS` and all four reserved IDs
  — `langlock.status`, `langlock.show`, `langlock.set`, `langlock.reset`
  (`scopesAllowed` global+project; `set`/`reset` mutating, offline-capable). The
  spec's C3 reference to catalog `1.0.0` is the clarify-time snapshot; the live
  version read from `RESERVED_CATALOG_VERSION` is authoritative. Finding: **no
  additive `1.3.0 -> 1.4.0` catalog bump is required** for Feature 004; it reuses
  the existing reserved surface and registers only typed domain impls and audit
  events (FR32, AC13).
