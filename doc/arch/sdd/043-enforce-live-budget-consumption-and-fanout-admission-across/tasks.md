...
- [x] T001 Confirm the seams and the engine surface by reading the code: the
  ...
  `ZERO_CONSUMPTION`.
- [x] T002 Add a pure consumption-accumulation helper
...
- [x] T003 Record real consumption at the response loop
...
- [x] T004 Re-evaluate the budget mid-session: after recording, call
...
- [x] T005 Feed real headroom into fan-out admission
...
- [x] T006 Reflect real consumption in the accounting
...
- [x] T007 Reconcile the out-of-box defaults (`config-adapter.ts`
...
- [x] T008 Make the budget path total and hang/crash-safe: wrap consumption
...
- [x] T009 Add the regression (`test/session/budget-consume.test.ts`) over the real
...
- [x] T010 Author the speckit corpus (`spec.md`, `plan.md`, `tasks.md`) and
